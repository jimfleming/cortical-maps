function [RetinaAllX,RetinaAllY,RetinaAllOD,RetinaAllONOFF,TotalRetinaCells,rf_space_x,rf_space_y] = make_retina5(distAPON,...
                    distMLON,distAPOFF,distMLOFF,OFFONDensityRatio,jitterON,jitterOFF,CrtxLength,MinDistONOFF,MaxDistONOFF,pix2mic,rng_trial,show_fig)
...
%     range_y = GridONXstep : GridONYstep : (GridONXstep * Width); 
    ...
    %range_x = linspace(distAPON,Width,num_cell_x);
...
%     [ONxGridRetina , ONyGridRetina] = meshgrid( -VisSpaceBoundX : GridONXstep : VisSpaceBoundX , -VisSpaceBoundY : GridONYstep : VisSpaceBoundY );
%     GridOFFXstep = distAPOFF * (1/OFFONDensityRatio);
%     GridOFFYstep = distMLOFF * (1/OFFONDensityRatio);
%     [OFFxGridRetina , OFFyGridRetina] = meshgrid( -VisSpaceBoundX : GridOFFXstep : VisSpaceBoundX , -VisSpaceBoundY : GridOFFYstep : VisSpaceBoundY );
    ...
    if show_fig == 1 
        ...
        measure_dist(RetONxIpsi,RetONyIpsi,RetOFFxIpsi,RetOFFyIpsi,rf_space_x,num_bin,...
            jitterON,jitterOFF,MinDistONOFF,MaxDistONOFF,pix2mic,'Ipsi',1);
        %[onon_dist_ipsi,off_dist_ipsi,on_off_dist_ipsi] = measure_dist(RetONxIpsi,RetONyIpsi,RetOFFxIpsi,RetOFFyIpsi);
        
        % show_RGC_distance(onon_dist_contra,off_dist_contra,on_off_dist_contra,num_bin)
        % show_RGC_distance(onon_dist_ipsi,off_dist_ipsi,on_off_dist_ipsi,num_bin)
        
        % Measuring the amount of overlap (RetinaGeneration121419b.m)
%         overlap_on_on_contra = measure_overlap(onon_dist_contra,RFONCenter);
%         overlap_off_off_contra = measure_overlap(off_dist_contra,RFONCenter);
%         overlap_on_off_contra = measure_overlap(on_off_dist_contra,RFONCenter);

        % show_RGC_distribution(RetONxContra,RetONyContra,RetOFFxContra,RetOFFyContra,rf_space_x,'Contra')
    end 
...
function measure_dist(RetONx,RetONy,RetOFFx,RetOFFy,rf_space_x,num_bin,...
    ...
    if show_fig == 1 
        ...
        annotation('textbox',[0.1 0.88 0.1 0.1],'String',title_text,'EdgeColor','none','fontsize',14)
