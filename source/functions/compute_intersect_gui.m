function [intersections, angles] = compute_intersect_gui(map1, map2, contour1_levels, contour2_levels, label_contours, least_polygon_side_num, color_map, figure_title, axes)
    %
...
%     end
    ...
    if show_fig == 1 
        ...
        %figure 
        % subplot(3,4,[1,2,5,6]), 
        imagesc(map1, 'Parent', axes(1)); 
        colormap(axes(1), color_map);
        title(axes(1), figure_title) % ,'fontsize',12),
        axis(axes(1), 'off');
        axis(axes(1), 'square');
        hold(axes(1), 'on'),
        contour(axes(1), map2, contour2_levels, 'k', 'LineWidth', 1);
%         caxis([-1 1])
         caxis(axes(1),  [min(map1(:)) max(map1(:))] )
...
%         bar(axes(2), x_ticks+5, percents, 'w'); 
%         ylabel(axes(2), "Percent"); xlabel(axes(2), 'Angle (deg)');
%         axis(axes(2), 'square'),
%         set(axes(2),'XTick',xtick,'YTick',ytick,'XTickLabel',xtick,'YTickLabel',ytick,'Box','off','TickDir','out'); 
        ...
        polar1(axes(2), ori_bin_rad, freqs_polar', 'k');
    end 
end
