function [CrtxONOFF3m,CrtxOD3m,Retinotopy3mIndPlot,Retinotopy3mIndex] = make_cortex(CrtxLength,CrtxWidth, ...
                                onoff_rf_space,od_rf_space,RetinotopyRFspace_plot,IndRetinaRfspace,cmap,rng_trial,show_fig) 
    ...
    %     Debug : OD ONOFF Retinotopy before sorting 
    if show_fig == 1 
        figure
        set(gcf, 'units', 'normalized','OuterPosition',[0 0 .9 .9])
        subplot(131),imagesc(CrtxOD3m), colormap(gca, 'gray'), axis square,title('Ocular Dominance')
        subplot(132),imagesc(CrtxONOFF3m), colormap(gca, 'jet'), axis square,title('ON OFF')
        subplot(133),imshow(Retinotopy3mIndPlot,cmap),title('Retinotopy')
        sgtitle('Cortical Plate before sorting ','fontsize',30)
    end
end 
