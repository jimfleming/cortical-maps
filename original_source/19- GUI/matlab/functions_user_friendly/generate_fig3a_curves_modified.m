function generate_fig3a_curves_modified(OriSFdata_contra,OriSFdata_Ipsi,ODCrtxPlt_smooth,electrode_position,eye_pref,rng_trial)
...
%     set(gcf, 'units', 'normalized','OuterPosition',[0 0 .9 .9]);
    ...
    for kk = 1:5
        ...
        title(sprintf('SF50:%.2f \n CV:%.2f \n LHI:%.2f',SF50,CV,LHI))
        xlabel(sprintf('row : %.0f \n col : %.0f',ii,jj))
        hold on, scatter(sf_bin_deg_points,sf_tuning_resp_points,'ko')
        ylim([ -0.1 1])
        xlim([0.05 1.5])
        ...
        subplot(3,7,[6.5 7])
        hold on
        plot(SF50,CV,'Marker','o','MarkerEdgeColor',plot_color,'MarkerFaceColor',plot_color);
        caxis([0 1])
        xlabel('SF50'), ylabel('CV') 
        ylim([0 1]),
        xlim([0 1.1]), 
        box off
        set(gca,'box','off')%,'Tickdir','out')
        dx = 0.00; dy = 0.07; % displacement so the text does not overlay the data points
        text(SF50+dx, CV+dy, num2str(kk-1));
        
        subplot(3,7,[13.5 14])
        hold on
        plot(SF50,LHI,'Marker','o','MarkerEdgeColor',plot_color,'MarkerFaceColor',plot_color);
        caxis([0 1])
        xlabel('SF50'), ylabel('LHI')
        ylim([0 1]),
        xlim([0 1.1]), 
        box off
        set(gca,'box','off')%,'Tickdir','out')
        text(SF50+dx, LHI+dy, num2str(kk-1));

        plot_psths(ODI_contra(ii,jj), ODI_ipsi(ii,jj), kk, 1) 
    end

    %% plot the cropped 2D maps of the selected locations 
    
    if debug == 1
        if strcmp(eye_pref(1),'c')
            OriMapPref = OriSFdata_contra.ori_map_smooth;
            LHI_map = OriSFdata_contra.LHI_map;
            CV_map = OriSFdata_contra.CV_map;
            SF50_map = OriSFdata_contra.SF50_map;
        elseif strcmp(eye_pref(1),'i')
            OriMapPref = OriSFdata_ipsi.ori_map_smooth;
            LHI_map = OriSFdata_ipsi.LHI_map;
            CV_map = OriSFdata_ipsi.CV_map;
            SF50_map = OriSFdata_Ipsi.SF50_map;
        end
        ...
        subplot(3,7,1); imagesc(OriMapPref);title('Orientation');colormap(gca,'hsv');axis square; axis off
        hold on, contour(ODCrtxPlt_smooth, 1, 'k', 'LineWidth', 1);
        xlim([x1 x2]), ylim([y1 y2])
        subplot(3,7,2); imagesc(LHI_map); title('LHI'); axis square; colormap(gca,'jet'); axis off
        hold on, contour(ODCrtxPlt_smooth, 1, 'k', 'LineWidth', 1);
        xlim([x1 x2]), ylim([y1 y2])
        subplot(3,7,3); imagesc(CV_map);axis square; title('CV'); axis off
        hold on, contour(ODCrtxPlt_smooth, 1, 'k', 'LineWidth', 1);
        xlim([x1 x2]), ylim([y1 y2])
        colormap(gca,'jet')
        ...
        xlim([x1 x2]), ylim([y1 y2])
        colormap(gca,'gray')
        subplot(3,7,5); imagesc(SF50_map);axis square; title('SF50'); axis off
        hold on, contour(ODCrtxPlt_smooth, 1, 'k', 'LineWidth', 1);
        xlim([x1 x2]), ylim([y1 y2])
        colormap(gca,'jet')
...
%     annotation('textbox',[0.03 0.82 0.08 0.08],'String',eye_pref_plot,'EdgeColor','none','fontsize',16)
%     annotation('textbox',[0.03 0.87 0.08 0.08],'String','fig3a','EdgeColor','none','fontsize',16)
...
%
% this should function should be modified 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
