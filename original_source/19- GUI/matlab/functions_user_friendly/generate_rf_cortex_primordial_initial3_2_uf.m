function [output_rf,output_data] = generate_rf_cortex_primordial_initial3_2_uf(pref_eye,RetinaRF,r_covered_aff,rowRange,colRange,ODCrtxPlt,ONOFFCrtxPlt, ... 
...
%             mask_spread = dist_matrix <= sdspread; %   figure,imagesc(mask_spread)
            ...
            [allCXrfTempDominant, aff_center_weight_plot] = gen_rf(RetinaRF, RetONOFFsorted, ONOFFCrtxPlt, i_aff_rf_space, inda, indb, tmpweights, dominant_pol, 1, aff_center_weight_plot, debug);
...
%             [center_rf_row,center_rf_col] = find(abs(rf_norm) == 1);
            ...
            if isnan(center_rf_row)
                ...
                hold on, viscircles([jj, ii], sdspread, 'color', 'k')
                colormap jet 
                error(['There is not enough Afferent reaching at this cortical location from ' pref_eye ' eye'])
            end 
            % check
...
%             temp_mask1 = imdilate(temp_mask1, mask_covered_aff);
            ...
            [allCXrfTempNonDominant, aff_center_weight_plot] = gen_rf(RetinaRF, RetONOFFsorted, ONOFFCrtxPlt, i_aff_rf_space, inda, indb, weight_other_pol, non_dominant_pol, 0, aff_center_weight_plot, debug);
...
function [allCXrf, aff_center_weight_plot] = gen_rf(RetinaRF, RetONOFFsorted, ONOFFCrtxPlt, i_aff_rf_space, row_ind, col_ind, weights, pol, ...
                                                    is_dominant, aff_center_weight_plot, debug)
