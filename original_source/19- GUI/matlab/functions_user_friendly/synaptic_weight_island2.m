...
%         
%         SelectedArea = ONOFFODLabelSorted == ONOFFODLabelSorted(i_row,i_col); %  figure,imagesc(SelectedArea)
%         OuterEdge = imdilate(~SelectedArea,ones(3)) - ~SelectedArea;  
%         [row_border,col_border] = find(OuterEdge); 
%         
...
% 
%         r_pw = round(pw_locations(i_pw,2));
%         c_pw = round(pw_locations(i_pw,1));
%         if SelectedArea(r_pw,c_pw)
%             weight_map(i_row,i_col) = (weight_pw * dist_border + weight_border * dist_pw) / (dist_border + dist_pw); 
%         else 
%             weight_map(i_row,i_col) = 1 ; 
%         end 
%     end 
% end 
%% 
