function [ori_tuning_resp,sf_tuning_resp,sf_tuning_resp_dog,cv,ori_pref_deg,sf_pref_deg,sf50_deg,LPI,angles_bin,num_cycles_bin_deg] = ... 
                    fft_ori_sf_tuning6(rf_input,sf_lSamp,gauss_fit_sfcurve,pix2deg,debug)
...
%      rf_input = rf_input .^ 3; 
%     rf_input2 = zeros(401); 
%     rf_input2(1:size(rf_input,1),1:size(rf_input,2)) = rf_input; 
    ...
    sf_tuning_resp = measure_sf_tuning_curve(img_fft_abs_norm_ori,ori_pref_rad,center_fft,sf_lSamp); 
    [sf_pref_deg,sf50_deg,LPI,sf_tuning_resp_dog,num_cycles_bin_deg] = sf_measurement(sf_tuning_resp,img_fft_abs,gauss_fit_sfcurve,sf_lSamp,pix2deg);

    % measuring sf max from another method 
    sf_pref_deg_from_radius = measure_sf_from_radius(img_fft_abs_norm_ori,center_fft,pix2deg);

    %% debug 
    if debug == 1
        figure(); clf
        set(gcf,'Units','Normalized','Outerposition',[0.1 0.1 .8 .8])
        subplot(221),imagesc(rf_input),colormap jet;colorbar,axis square
        caxisVal = 1; 
        caxis([-caxisVal caxisVal])
        subplot(222),imagesc(img_fft_abs2),colormap jet;colorbar,axis square
        subplot(223),polar1(angles_bin,ori_tuning_resp','k');hold on;axis square        
        title(sprintf('Ori : %.2f ',ori_pref_deg))

        subplot(224),plot(num_cycles_bin_deg,sf_tuning_resp_dog,'lineWidth',2);axis square
        set(gca, 'XScale', 'log');
        title(sprintf('SF : %.2f CPD \n SF50 : %.2f ',sf_pref_deg,sf50_deg))        
        %title(sprintf('SF : %.2f CPD SF2 : %.2f CPD \n SF50 : %.2f ',sf_pref_deg,sf_pref_deg_from_radius,sf50_deg))
        set(gca,'box','off','Tickdir','out');%,'YTickLabel',[],'XTickLabel',[])
        %sgtitle(sprintf('%.0f',jj),'fontsize',20)
        %pause
    end
end
